...
try {
  ...
  const forbidden = ['src/cli.ts', 'tests/cli.run.test.ts', '.github/workflows/ci.yml', '.env', 'dist/cli.js.map'];
  ...
  const json = run(
    process.execPath,
    [cli, 'run', '.roleplay/scenarios/install-smoke.yml', '--json'],
    { cwd: installDir },
  );
  ...
  const packagedCli = readFileSync(cli, 'utf8');
  for (const forbiddenText of [
    'refund-policy-edge-case',
    'prompt-injection-basic',
    'support-happy-path',
    'authority-impersonation-compliance-refund',
  ]) {
    if (packagedCli.includes(forbiddenText)) {
      throw new Error(`Packaged CLI leaked private scenario content: ${forbiddenText}`);
    }
  }
  ...
  if (!upload.runUrl.startsWith(`${endpoint}/runs?run=`)) {
    throw new Error('Packaged CLI did not return an absolute workbench run URL.');
  }
