...
describe('cli init', () => {
  it('creates a workbench-ready starter project with subscription placeholders', async () => {
    ...
    expect(envExample).toContain('ROLEPLAY_CLOUD_URL=https://app.roleplay.sh');
    expect(envExample).toContain('ROLEPLAY_PROJECT_ID=');
    expect(envExample).toContain('ROLEPLAY_API_KEY=');
    expect(envExample).toContain('ROLEPLAY_AGENT_NAME=');
    ...
    await expect(readFile(join(cwd, '.roleplay', 'scenarios', 'install-smoke.yml'), 'utf8')).resolves.toContain(
      'name: install-smoke',
    );
  });
