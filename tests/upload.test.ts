...
import {
  buildUploadPayload,
  assertRunEntitlement,
  assertUploadEntitlement,
  requireUploadApiKey,
...
describe('upload client', () => {
  ...
  it('surfaces workbench authentication failures clearly', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn(async () => {
  ...
  it('verifies project API keys without uploading a run', async () => {
    ...
    expect(verification).toMatchObject({
      ...
      entitlement: { plan: 'builder', status: 'trialing', canRun: true, canUpload: true },
    });
  });

  it('rejects inactive run and upload entitlements with billing guidance', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn(async () => {
        return new Response(
    ...
    await expect(assertRunEntitlement(input)).rejects.toMatchObject({
    ...
    await expect(assertUploadEntitlement(input)).rejects.toMatchObject({
      code: 'WORKBENCH_SUBSCRIPTION_INACTIVE',
    });
  });
  ...
  it('surfaces workbench API key verification failures clearly', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn(async () => {
        return new Response(JSON.stringify({ error: 'A valid project API key is required.' }), {
  ...
  it('surfaces workbench network failures clearly', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn(async () => {
    ...
    await expect(
      ...
      code: 'UPLOAD_FAILED',
      message: 'Could not reach workbench at http://127.0.0.1:9.',
      suggestion: 'Check ROLEPLAY_CLOUD_URL, ROLEPLAY_API_KEY, and that workbench is running.',
    });
  });

  it('rejects malformed successful workbench upload responses', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn(async () => {
  ...
  it('rejects impossible successful workbench upload counts', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn(async () => {
        return new Response(
  ...
  it('rejects successful workbench upload responses with external report URLs', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn(async () => {
        return new Response(
  ...
  it('rejects successful workbench upload responses that do not match the requested upload', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn(async () => {
        return new Response(
  ...
  it('rejects contradictory local reports before sending them to workbench', async () => {
    const runsDir = await mkdtemp(join(tmpdir(), 'roleplay-upload-'));
    try {
  ...
  it('rejects invalid local report timestamps before sending them to workbench', async () => {
    const runsDir = await mkdtemp(join(tmpdir(), 'roleplay-upload-'));
    try {
    ...
    expect(() =>
      cloudUploadSchema.parse({
        ...
        run: {
          ...
          transcript: {
            runId: 'run_upload_test',
            scenarioName: 'local-smoke-scenario',
            startedAt: '2026-05-31T10:00:00.000Z',
            turns: [
              {
                turn: 1,
